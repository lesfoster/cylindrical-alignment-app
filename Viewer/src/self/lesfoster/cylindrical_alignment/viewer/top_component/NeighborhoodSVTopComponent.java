...
public final class NeighborhoodSVTopComponent extends TopComponent {
    ...
    private void initComponents() {
        ...
        layout.setHorizontalGroup(
            ...
            .addComponent(neighborhoodPanel, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
        );
        ...
        private void report(Map<Object,Object> modelInfo) {
            ...
            for (int i = Math.max(0, residuePos - MAX_CLUSTER); i <= residuePos + MAX_CLUSTER; i++) {
                ...
                Optional.ofNullable(residueData.getDentilPosToIds().get(i))
                        .ifPresent(l -> neighbors.add(new Neighbor(pos, l, getAllTypes(pos))));
            }
            neighborhoodTableModel.setNeighbors(neighbors, residuePos);
        }
    }
    
    private List<Polymorphism.Type> getAllTypes(int pos) {
        return residueData.getDentilPosToIds().get(pos).stream()
                .map(id -> residueData.getIdToPolymorphism().get(id).getType())
                .collect(toCollection(() -> new HashSet<Polymorphism.Type>()))
